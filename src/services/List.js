...
const List = {
    ...
    addRecipe: async (listId, recipe, userId) => {
        const { data } = await axios.post(`/users/${userId}/lists/${listId}/recipes`, recipe);
        return data;
    },
    // Create new list
    create: async (listName, userId) => {
        const { data } = await axios.post(`/users/${userId}/lists`, { name: listName });
        return data;
    },
    ...
    deleteRecipe: async (listId, recipeId, userId) => {
        const { data } = await axios.delete(`/users/${userId}/lists/${listId}/recipes/${recipeId}`);
        return data;
    },
    // Edit the name of a list
    editName: async (listId, newName, userId) => {
        const { data } = await axios.put(`/users/${userId}/lists/${listId}`, { name: newName });
        return data;
    },
    ...
    getOne: async (listId, userId) => {
        const { data } = await axios.get(`/users/${userId}/lists/${listId}`);
        return data;
    }
}
